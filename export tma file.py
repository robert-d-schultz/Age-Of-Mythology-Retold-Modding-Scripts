...
#Use "static" mode for bones that are static (this one will reduce file size by like 50%)
...
with open(tma_output_filename, "wb") as tma_file:
    ...
    #"Import Crap"
    tma_file.write(struct.pack("L", 4)) #bytelength of the imports, 4 for the int below
    tma_file.write(struct.pack("L", 0)) #(none)
    
    tma_file.write(struct.pack("L", len(bones))) #"active_bone_count", assume all are active for now
    ...
    for bone in bones:
        ...
        tma_file.write(struct.pack("ffffffffffffffff", *parent_space_mat_list)) #parent space 4x4
        tma_file.write(struct.pack("ffffffffffffffff", *world_space_mat_list))  #world space 4x4
        tma_file.write(struct.pack("ffffffffffffffff", *inverse_bind_mat_list)) #inverse bind pose
    ...
    for pose_bone in arm_obj_eval.pose.bones:
        ...
        for i in range(frame_count):
            ...
            bpy.context.scene.frame_set(i)#calling it twice fixes... something? maybe?
        ...
        for i in range(frame_count):
            ...
            max_index = abs_components.index(max(abs_components))  # Index of largest component
            ...
            for i,c in enumerate(scaled):
                c *= (1 if (i == 0 or max_index == 3) else -1) #Weird thing, deal with it
                sign_bit = 0 if c < 0 else 1
                magnitude = int(min(abs(c), 1.0) * ((1 << 19) - 1))  # Map [0,1] to [0, 2^19 - 1]

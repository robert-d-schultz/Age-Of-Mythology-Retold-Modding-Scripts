...
if not armature_object:
    ...
    print(f"Active object '{armature_object.name}' is not an armature. It will be ignored and a new armature will be created.")
    armature_object = None
else:
...
with open(tma_filename, "rb") as anim_file:
    ...
    assert num_active_bones <= num_bones, f"More active bones ({num_active_bones}) than total bones ({num_bones}), impossible."
    ...
    for i in range(num_bones):
        ...
        try:
            eb = ebs[bone_name]
            print(f"    Already in armature, skipping")
        except KeyError:
            ...
            if i > 0:
                try:
                    ...
                    raise Exception(f"    No parent found, impossible.")
            else:
                pass
    ...
    for _ in range(num_active_bones):        
        ...
        assert unknown_one == 1, "  Shouldn't this always be 1? byte: " + anim_file.tell()
        
        position_mode = struct.unpack("<B", anim_file.read(1))[0] #1=normal, 0=static position
        rotation_mode = struct.unpack("<B", anim_file.read(1))[0] #3=normal, 0=static rotation
        
        unknown_zero = struct.unpack("<B", anim_file.read(1))[0]
        assert unknown_zero == 0, "  Shouldn't this always be 0? byte: " + anim_file.tell()
        
        
        num_frames = struct.unpack("<L", anim_file.read(4))[0]
        assert num_frames == num_frames_global, "  num_frames doesn't match num_frames_global"
        ...
        def read_rotation(raw):
            ...
            a,b,c = min(a*norm_scale*quat_factor,1), min(b*norm_scale*quat_factor,1), min(c*norm_scale*quat_factor,1)
        ...
        rotations = []
        if rotation_mode == 0:
            static_z,static_y,static_x,static_w = struct.unpack("ffff", anim_file.read(16))
            rotations.extend([mathutils.Quaternion((-static_w,static_z,static_y,static_x))] * num_frames)
    ...
    for _ in range(num_attachs):
        ...
        time = struct.unpack("<f", anim_file.read(4))[0] #doesn't make sense as time in seconds, could be % of frames

...
with open(tmm_filename, "rb") as tmm_file:
    ...
    bigger_bounding_box = struct.unpack(
        "<ffffff", tmm_file.read(24)
    )  # maximum range from anims?

    unknown_float = struct.unpack("<f", tmm_file.read(4))[
        0
    ]  # HP bar height or something? seems a bit low

    # Counts of things
    num_mesh_groups = struct.unpack("<L", tmm_file.read(4))[0]
    num_materials = struct.unpack("<L", tmm_file.read(4))[0]
    num_defaults = struct.unpack("<L", tmm_file.read(4))[
        0
    ]  # "default"s, probably shader techniques
    ...
    for _ in range(num_attachments):
        ...
        transform_matrix1 = struct.unpack(
            "<ffffffffffff", tmm_file.read(48)
        )  # 4x3 matrix
        transform_matrix2 = struct.unpack(
            "<ffffffffffff", tmm_file.read(48)
        )  # 4x3 matrix

        unknown_zero1 = struct.unpack("<L", tmm_file.read(4))[
            0
        ]  # unknown, always 0, sometimes 2 for relics/riders?
    ...
    for _ in range(num_mesh_groups):
        ...
        defaut_index = struct.unpack("<L", tmm_file.read(4))[
            0
        ]  # shader technique from below?
    ...
    for _ in range(num_bones):
        ...
        radius = struct.unpack("<f", tmm_file.read(4))[
            0
        ]  # radius for collision (needed for clicking on the unit)
...
def read_tmm_data(
    tmm_data_filename, num_vertices, num_triangles, mesh_group_list, armature_object
):
    with open(tmm_data_filename + ".data", "rb") as tmm_data_file:
        ...
        for i, mesh_group in enumerate(mesh_group_list):
            ...
            for j in range(mesh_group[0]):
                ...
                (norm_x, norm_y, norm_z) = oct_decode(norm_a, norm_b) * (
                    1 if norm_c == 0 else -1
                )
                (tan_x, tan_y, tan_z) = oct_decode(tan_a, tan_b) * (
                    1 if tan_c == 0 else -1
                )
                (bitan_x, bitan_y, bitan_z) = oct_decode(bitan_a, bitan_b) * (
                    1 if bitan_c == 0 else -1
                )
        ...
        if armature_object is not False:
            for vertex in range(num_vertices):
                ...
                weighted_bones = [
                    (bone_id, weight)
                    for bone_id, weight in zip(bone_ids, weights, strict=False)
                    if weight != 0
                ]
        ...
        current_mesh.uv_layers[-1].data.foreach_set(
            ...
            [
                uv
                for pair in [uv_list[loop.vertex_index] for loop in current_mesh.loops]
                for uv in pair
            ],
        )
...
mesh_obj = read_tmm_data(
    tmm_filename, num_vertices, num_triangles, mesh_group_list, armature_obj
)
